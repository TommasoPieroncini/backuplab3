...
boot_alloc(uint32_t n)
{
  ...
  void *start = (void *) nextfree;
  if (n == 0) {
    return start;
  }
  int pgNeeded = n / PGSIZE;
  if (pgNeeded > npages) {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  if (!(*pgdir & PTE_P)) {
    if (create) {
      ...
      newpage->pp_ref++;
      *pgdir = page2pa(newpage) | PTE_P | PTE_U | PTE_W;
    } else {
      return NULL;
    }
  }
  pte_t *pte = KADDR(PTE_ADDR(*pgdir));
  return pte + PTX(va);
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
  ...
  for (int i = 0; i < size / PGSIZE; i++) {
    ...
    if (!pte) {
      panic("out of memory for mapping\n");
    }
    *pte = pa | perm | PTE_P;
    va += PGSIZE;
    pa += PGSIZE;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  // LAB 3: Your code here.
	...
	for (int i = (int) ROUNDDOWN(va, PGSIZE); i < (int) ROUNDUP(va + len, PGSIZE); i += PGSIZE) {
